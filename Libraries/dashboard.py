...
def get_burned_calories_total(df_user, peso_kg):
   ...
   #st.text(df_calculated['Total_kcal_burned'].sum())
   #st.text(df_calculated['Time_Minutes_diff'].sum())
   ...
   #total_minutes = df_calculated['Time_Minutes_diff'].sum()
...
def plot_all_aprox_gauge_chart(df_results):
   ...
   for i, row in df_results.iterrows():
      #class_pose = str(row['Class']).upper()
      value = row['Prob']
